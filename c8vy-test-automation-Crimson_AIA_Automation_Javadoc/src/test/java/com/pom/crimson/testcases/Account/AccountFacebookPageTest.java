 ...
 */	
public class AccountFacebookPageTest extends BaseFixture {
	LoginPage loginPage;
	HomePage homePage;
	LogAssert logAssert;
	...
	@Test(enabled=true, priority = 83, testName = "Verify all the menus are working in Account screen via Facebook", description = "Verify all the menus are working in Account screen via Email")
		public void VerifyManusInAccountScreenviaFacebook() throws InterruptedException, IOException
		{ 
    ...
    MP.clickManageProfile();
    logAssert.assertTrue(MP.isElementDisplayed("Manage profiles"),"Manage profile screen appears");
	logAssert.assertTrue(MP.isElementDisplayed(Constants.FBTestUser),"Logged in with correct Email user");
    logAssert.assertTrue(MP.isElementDisplayed("Manage profiles"),"Profile screen appears");
	CP.BackButton();
    AccountPage AP = new AccountPage(getDriver(), getPlatformName());
    AP.ClickConsultations();
