...
/**
 * This class contains test methods to test <b>Logout</b> functionality via Google Login 
 * 
 * @author Balaji.Sridharan 
 */	
public class LogoutGooglePageTest extends BaseFixture{
	...
	@Test(enabled=true, priority = 252, testName = "Verify Log out with Google", description = "Verify Log out with Google")
	public void VerifyLogoutviaGoogle() throws Exception
	{
		ExtentReportManager.getTest().log(Status.INFO, "Verify Log out with Google");
		loginPage.Takepic_Recvideo();
		loginPage.skipIntroScreens();
		logAssert.assertTrue(loginPage.isElementDisplayed("Continue with Google"),"Continue with Google option appears in Login or Sign up screens");
