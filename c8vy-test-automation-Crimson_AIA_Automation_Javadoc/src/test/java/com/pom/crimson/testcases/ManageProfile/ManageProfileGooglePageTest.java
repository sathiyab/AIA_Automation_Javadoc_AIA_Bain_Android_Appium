...
/**
 * This class contains test methods to test <b>Manage profile</b> functionality via Google Login 
 * 
 * @author Balaji.Sridharan 
 */	
public class ManageProfileGooglePageTest extends BaseFixture {
	LoginPage loginPage;
	HomePage homePage;
	LogAssert logAssert;
...
@Test(enabled=true, priority = 191, testName = "Verify switching from We are planning to have a baby to We are pregnant flow via Google login", description = "Verify switching from \"We are planning to have a baby\" to \"We are pregnant flow\" with No Option via Google login")
public void OnboardingPHBSwitchArchetypeviaGoogle() throws InterruptedException, IOException
{ 
...
@Test(enabled=true, priority = 192, testName ="Verify switching from My partner and I are Pregnant to I have a new born via Google", description = "Verify switching from \"My partner and I are Pregnant\" to \"I have a new born\"via Google")
public void OnboardingPRGSwitchArchetypedMaleFlowviaGoogle() throws InterruptedException, IOException
{
 loginPage.Takepic_Recvideo();
...
logAssert.assertAll();
 ...
 @Test(enabled=true, priority = 193, testName =" Verify switching from I have a new born to I have a child/children via Google", description = "Verify switching from \"My partner and I are Pregnant\" to \"I have a new born\"via Google")
public void SwitchArchetyped_NB_to_CC_viaGoogle() throws InterruptedException, IOException
{
	 loginPage.Takepic_Recvideo();
...
logAssert.assertAll();
 ...
 @Test(enabled=true, priority = 194, testName ="Verify switching from I have a child children to We are planning to have a baby via Google", description = "Verify switching from \"My partner and I are Pregnant\" to \"I have a new born\"via Google")
	public void SwitchArchetyped_CC_to_PHB_viaGoogle() throws InterruptedException, IOException
	{
	 loginPage.Takepic_Recvideo();
	...
	logAssert.assertAll();
	 ...
	 @Test(enabled=true, priority = 195, testName = "Verify Update Archetype for We are planning to have a baby via Google", description = "Verify Update Archetype for We are planning to have a baby via Google")
		public void PHBUpdateArchetypedMaleFlowviaGmail() throws InterruptedException, IOException
		{ 
		loginPage.Takepic_Recvideo();
		loginPage.skipIntroScreens();
		loginPage.LoginWithGmail();	
